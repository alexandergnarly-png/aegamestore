...
assert.match(html, /id="backgroundMusic"[\s\S]*?romance\.m4a\?v=20260814-lite-1[\s\S]*?preload="none"[\s\S]*?loop/);
...
assert.ok(fs.statSync("public/assets/audio/romance.m4a").size < 9 * 1024 * 1024);
...
assert.match(serviceWorker, /CACHE_VERSION = "20260814-bandwidth-v1"/);
