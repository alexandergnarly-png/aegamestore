...
assert.doesNotMatch(html, /id="backgroundMusic"|id="musicToggle"|romance\.m4a/);
assert.doesNotMatch(script, /setupBackgroundMusic|ae_music_enabled/);
assert.doesNotMatch(script, /setInterval\(loadRecentPurchases/);
...
assert.match(css, /AE patch 20260825-checkout-docket-v1/);
assert.match(css, /\.order-modal \.order-product-cards \{[\s\S]*?scroll-snap-type: x mandatory/);
...
[
  ...
  "installAppKicker",
  "installAppReady",
  "installAppBenefitFast",
  "installAppBenefitLight",
].forEach((key) => {
...
assert.match(html, /script\.js\?v=20260825-checkout-docket-v1/);
assert.match(html, /style\.css\?v=20260825-checkout-docket-v1/);
...
assert.match(serviceWorker, /CACHE_VERSION = "20260825-checkout-docket-v1"/);
...
assert.match(keysystemCss, /\.header-inner \{[\s\S]*?min-height: 60px;[\s\S]*?padding: 6px 10px/);
assert.match(keysystemCss, /\.header-inner \.brand-logo \{[\s\S]*?width: 38px;[\s\S]*?border-radius: 13px/);
assert.match(keysystemCss, /\.lang-switch \{[\s\S]*?border-radius: 15px !important;[\s\S]*?background: #ffffff !important/);
assert.match(keysystemCss, /\.nav-toggle::before \{[\s\S]*?inset: 3px;[\s\S]*?border-radius: 13px;[\s\S]*?background: #ffffff/);
...
assert.match(css, /\.ae-ai-disclosure\[open\] \.ae-ai-summary-action/);
assert.match(css, /\.sr-only \{[\s\S]*?clip: rect\(0, 0, 0, 0\)/);
assert.match(script, /aiDisclosure\?\.removeAttribute\("open"\)/);
